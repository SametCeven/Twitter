...
public class CommentRepositoryTest {
    ...
    @DisplayName("getLikeCount, can it return correct number of likes of a comment with given comment id")
    @Test
    void getLikeCount(){

        when(commentRepository.getLikeCount(comment.getId()))
                .thenReturn(1);

        Integer foundLikeCount = commentRepository.getLikeCount(comment.getId());

        Assertions.assertEquals(1, foundLikeCount);

    }

    @DisplayName("getRetweetCount, can it return correct number of retweet of a comment with given comment id")
    @Test
    void getRetweetCount(){

        when(commentRepository.getRetweetCount(comment.getId()))
                .thenReturn(1);

        Integer foundRetweetCount = commentRepository.getRetweetCount(comment.getId());
